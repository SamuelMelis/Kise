...
import { DollarSign, Briefcase, Plus, Trash2, Landmark, Bitcoin, TrendingUp, Home, Box, Wallet, X, CreditCard } from 'lucide-react';
...
export const IncomeTab: React.FC = () => {
  ...
  const totalStable = stableIncomes.reduce((sum, i) => sum + i.amount, 0);
  const totalVariable = variableIncomes.reduce((sum, i) => sum + i.amount, 0);
  const totalIncome = totalStable + totalVariable;
  const totalAssets = assets.reduce((sum, a) => {
    return a.type === 'Debt' ? sum - a.amount : sum + a.amount;
  }, 0);
            ...
            <form onSubmit={handleAddIncome} className="space-y-4">
              <div className="grid grid-cols-2 gap-2 bg-white p-1 rounded-xl border border-gray-100">
                <button
                  ...
                  className={`py-3 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${type === 'Stable' ? 'bg-[#18181b] text-white shadow-md' : 'text-gray-400 hover:text-[#18181b]'}`}
                >
                  Stable
                </button>
                <button
                  ...
                  className={`py-3 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${type === 'Variable' ? 'bg-[#18181b] text-white shadow-md' : 'text-gray-400 hover:text-[#18181b]'}`}
                >
                  Variable
                </button>
              </div>
      ...
      <div className="space-y-8">
        ...
        <div>
          <h3 className="flex items-center gap-2 text-[#18181b] font-bold text-xs uppercase tracking-widest mb-4">
            <Briefcase size={14} /> Stable Income
          </h3>
          <div className="space-y-3">
            {stableIncomes.map(income => (
              <div key={income.id} className="bg-white p-4 rounded-2xl border border-gray-100 flex justify-between items-center group hover:border-[#18181b] transition-all">
                ...
                <div className="flex items-center gap-4">
                  <span className="font-mono font-bold text-[#18181b] tracking-tight">${income.amount.toLocaleString()}</span>
            ...
            {stableIncomes.length === 0 && <div className="text-gray-300 text-xs italic pl-2">No stable sources.</div>}
          </div>
        </div>
        ...
        <div>
          <h3 className="flex items-center gap-2 text-[#18181b] font-bold text-xs uppercase tracking-widest mb-4">
            <DollarSign size={14} /> Variable Income
          </h3>
          <div className="space-y-3">
            {variableIncomes.map(income => (
              <div key={income.id} className="bg-white p-4 rounded-2xl border border-gray-100 flex justify-between items-center group hover:border-[#18181b] transition-all">
                <div>
                  <div className="font-bold text-[#18181b] text-sm">{income.source}</div>
                  <div className="text-[10px] text-gray-400 font-medium">{income.date}</div>
                </div>
                <div className="flex items-center gap-4">
                  <span className="font-mono font-bold text-[#18181b] tracking-tight">+${income.amount.toLocaleString()}</span>
                  <button onClick={() => deleteIncome(income.id)} className="text-gray-300 hover:text-[#18181b] transition-colors">
            ...
            {variableIncomes.length === 0 && <div className="text-gray-300 text-xs italic pl-2">No variable sources.</div>}
          </div>
        </div>
            ...
            <form onSubmit={handleAddAsset} className="bg-gray-50 p-5 rounded-2xl border border-gray-200 mb-6 animate-in fade-in slide-in-from-top-2">
              ...
              <div className="space-y-3">
                <input
                  ...
                  placeholder="Asset Name (e.g. Bitcoin, Cash)"
            ...
            {assets.map(asset => {
              ...
              return (
                <div key={asset.id} className="bg-white p-4 rounded-2xl border border-gray-100 flex justify-between items-center group hover:border-[#18181b] transition-all">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center text-[#18181b] border border-gray-200 group-hover:border-[#18181b] transition-colors">
                      <Icon size={18} strokeWidth={1.5} />
                    </div>
                    <div>
                      <div className="font-bold text-[#18181b] text-sm">{asset.name}</div>
                      <div className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">{asset.type}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <span className="font-mono font-bold text-[#18181b] tracking-tight">${asset.amount.toLocaleString()}</span>
